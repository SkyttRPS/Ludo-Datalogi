...
class SpilController {
    ...
    void opdaterFelt(int feltNR, int spillerNR,int brikID){
        ...
        //int usikkerBrik = spillere.sp.get(board.felter.get(feltNR-1).getSpillerPåFelt()).brikker.get(brikID-1).getBrikID();
        ...
        if (plads.tjekBrik()){
            ...
            if (plads.erFeltSikkert(feltNR)){
                ...
                spillere.slåHjem(plads.getSpillerPåFelt(), plads.brikPåFelt);
                System.out.println("Du har slået " + spillere.spillerensFarve(plads.getSpillerPåFelt()) + " hjem til første felt -Wuuhuu- ");
            }
            // Sætter brik på felt
        } else {
            board.setFeltOptaget(spillere.sp.get(spillerNR-1).brikker.get(brikID-1).getLokation(),spillerNR,spillere.sp.get(spillerNR-1).brikker.get(brikID-1).getBrikID());
        }
    }
    ...
    public int AIrul(){
        Random r = new Random();
        terning.rul();
        int brik =r.nextInt(4)+1;
        System.out.println("AIrul giver: "+ brik);
        return brik;
    }
}